...
double scaleWidth = 1.0; // Scale image width
double scaleHeight = 1.0; // Scale image height
int spacing = 800/4;

public Dory() {
forward = getImage("/imgs/Dor.png");//Load the image for the

// Initialize size, position, and velocity
width = (int) (60*scaleWidth); // Set width from image dimensions
height =(int) (50*scaleHeight); // Set height from image dimensions
...
public Dory(int x, int y, int spacing, int vx) {
	this();
	this.x=x;
	this.y=y;
...
try {
URL imageURL = Dory.class.getResource(path);
if(imageURL!=null) {
